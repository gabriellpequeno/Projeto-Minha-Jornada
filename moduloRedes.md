...
| Falha no ponto central paralisa toda a rede. | Falha em um único link ou nó pode paralisar a rede; difícil de reconfigurar. | Falha no cabo principal paralisa a rede; difícil de isolar falhas. | Extremamente cara e complexa de instalar e gerenciar. |
  ...
  ### Arquitetura de Rede (Modelos OSI e TCP/IP)
  A **Arquitetura de Rede** é a estrutura lógica que define como os dispositivos e protocolos se organizam para permitir a comunicação. Os modelos mais importantes são o Modelo OSI (Open Systems Interconnection) e o Modelo TCP/IP.

O Modelo OSI é um modelo conceitual de 7 camadas, usado como referência para entender o fluxo de comunicação:
  
| Camada OSI | Função Principal |
|--|--|
| 7. Aplicação | Interface com o usuário e serviços de rede (HTTP, FTP, SMTP) |
| 6. Apresentação | Formatação, compressão e criptografia/descriptografia de dados |
| 5. Sessão | Gerenciamento de sessões de comunicação e sincronização |
| 4. Transporte | Segmentação, controle de fluxo e confiabilidade (TCP, UDP) |
| 3. Rede | Endereçamento lógico e roteamento (IP) |
| 2. Enlace | Endereçamento físico (MAC) e detecção de erros |
| 1. Física | Transmissão de bits brutos (cabos, conectores) |

O **Modelo TCP/IP** é o modelo prático de 4 camadas que impulsiona a Internet, sendo mais conciso e funcional:

| Camada TCP/IP | Camadas OSI Correspondentes | Protocolos Chave |
|--|--|--|
| Aplicação | Aplicação, Apresentação, Sessão | HTTP, FTP, DNS, SMTP |
...
|--|--|--|
| Bem Conhecidas| 0 a 1023 | 80 (HTTP), 443 (HTTPS), 22 (SSH), 53 (DNS) |
| Registradas | 1024 a 49151 | Usadas por aplicativos e serviços registrados (IANA) |
| Dinâmicas/Privadas | 49152 a 65535| Usadas para comunicações temporárias (clientes) |

  

## Parte 4 | Desempenho de Rede e Nomenclatura de Rede
...
Inserção de código malicioso (ex: SQL Injection, XSS) para executar ações não autorizadas.

-  **Ameaças Internas:**

Atividades prejudiciais realizadas por usuários autorizados (funcionários, contratados).
...
## Parte 6 | Novas Tendências Tecnológicas
